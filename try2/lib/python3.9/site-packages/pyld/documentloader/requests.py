...
def requests_document_loader(secure=False, **kwargs):
    ...
    def loader(url, options=None, prev_url=None):
        """
        Retrieves JSON-LD at the given URL.

        :param url: the URL to retrieve.
        :param options: loader options
        :param prev_url: holder for
        ...
        if not any([pieces.scheme, pieces.netloc]):
            if prev_url:
                url = urllib_parse.urljoin(prev_url[-1], url)
                pieces = urllib_parse.urlparse(url)
    ...
    prev_url_stack = []
    return lambda url, options=None: loader(url, options, prev_url_stack)
